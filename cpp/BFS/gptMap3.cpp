...
int main() {
    ...
    for (int i = 1; i < N - 1; ++i) {
        if (maze[i][1] == 0)     border.push_back({i, 0});
        if (maze[i][N-2] == 0)   border.push_back({i, N-1});
        if (maze[1][i] == 0)     border.push_back({0, i});
        if (maze[N-2][i] == 0)   border.push_back({N-1, i});
    }
    ...
    maze[exitp.first][exitp.second] = 0;
